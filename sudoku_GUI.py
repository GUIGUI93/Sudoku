    ...
    while(1):
        ...
        # 如果没有可以填的选择，那么将上一个数字换成另一个选择
            ...
            while(len(last_options) == 0):
                ...
                # 往前回溯的话必须要将已经填入的数字清零，和limit_solve算法有关
            if flag_over:
                break
            puzzle[pos_blank[index]] = last_options.pop(0)
            stack_options.append(last_options)
...
def sudoku_verify(test):
    ...
    while(1):
        ...
        if len(options) > 0:
            ...
            if index == blank_nb:
                map_solved.append(puzzle[:])
                # print("stack_options: ", stack_options)
                last_options = []
                while (len(last_options) == 0):
                    if not stack_options:  # 说明已经用尽了可能了
                        flag_over = True
...
class SudokuGUI:
    ...
    def sudokuPuzzleWait(self):
        if self.level.get() != 'easy' and self.level.get() != 'median' and self.level.get() != 'hard':
            if self.flag_language == 'ch':
                messagebox.showwarning("检查难度", "难度必须是 easy, median 或者 hard 哦!")
            else:
                messagebox.showwarning("check difficulty", "The difficulty must be easy, median or hard!")
            return
        self.frame.destroy()
        self.frame = Frame(self.window, width = WIDTH, height = HEIGHT)
        self.background_label = Label(self.frame, image = self.background_image)
    ...
    def submit(self):
        ...
        if len(self.answer) != len(response):
            if self.flag_language == 'ch':
                messagebox.showwarning("检查答案", "还有漏填的空，或者空格中的数字超过9了...")
            else:
                messagebox.showwarning("check answer", "You haven't filled all blanks or some number is bigger than 9...")    
            return
            
        if self.answer == response:
            if self.flag_language == 'ch':
                messagebox.showinfo("检查答案","棒极了，答案完全正确！")
            else:
                messagebox.showinfo("check answer","Great, the answer is correct!")
        else:
            if self.flag_language == 'ch':
                messagebox.showwarning("检查答案", "答案有错误哦。。。")
            else:
                messagebox.showwarning("check answer", "Sorry, the answer is not correct...")
        return  
    ...
    def getAnswer(self):
        ...
        if self.flag_language == 'ch':
            flag = messagebox.askokcancel(message = "你真的想看答案吗?") 
        else:
            flag = messagebox.askokcancel(message = "Do you really want to see the answer?") 
        if flag:
            pass
        else:
            self.flag_count_timeOut = False
    ...
    def solve(self):
        ...
        for one in values:
            if one in all:
                ...
                if self.flag_language == 'ch':
                    messagebox.showwarning("检查题目", "题目中好像出现了奇怪的东西呢，只能填1到9的数字哦")
                else:
                    messagebox.showwarning("check puzzle", "Something wrong with the puzzle, you can only enter the number from 1 to 9")
                return
        if puzzle_verify(puzzle):
            solved = sudoku_solving(puzzle)
        else:
            if self.flag_language == 'ch':
                messagebox.showwarning("检查题目", "题目有错误哦，你可能做到了假数独。。。")
            else:
                messagebox.showwarning("check puzzle", "Something wrong with the puzzle, it may be a fake puzzle...")
            return
            
        if len(solved) == 0:
            if self.flag_language == 'ch':
                messagebox.showwarning("检查题目", "这个题目没有解，你可能做到了假数独。。。")
            else:
                messagebox.showwarning("check puzzle", "Something wrong with the puzzle, it may be a fake puzzle...")
            return
        elif len(solved) > 1:
            if self.flag_language == 'ch':
                messagebox.showwarning("检查题目", "这个题目有多个解，你可能做到了假数独。。。")
            else:
                messagebox.showwarning("check puzzle", "This puzzle has plenty solutions, it may be a fake puzzle...")
